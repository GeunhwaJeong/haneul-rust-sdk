//! Encoding and decoding helpers for the serialized Haneul private key formats.
//!
...
//! payload has two wire encodings, mirroring the Haneul CLI and the `haneul-types`
//! crate:
//!
//! - `bech32`: a Bech32 (BIP-173) string with the human-readable part
//!   `haneulprivkey`, produced by `haneul keytool generate` and `haneul keytool
//!   export`.
//! - `base64`: a plain Base64 string, the legacy encoding used for entries
//!   of the Haneul CLI's `haneul.keystore` file and by older versions of `haneul
//!   keytool`.
//!
...
fn hrp() -> bech32::Hrp {
    ...
    bech32::Hrp::parse(HRP).expect("`haneulprivkey` is a valid Bech32 HRP")
}
