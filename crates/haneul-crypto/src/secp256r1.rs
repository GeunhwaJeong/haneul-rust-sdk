...
impl Secp256r1PrivateKey {
    ...
    pub fn from_haneulprivkey(s: &str) -> Result<Self, SignatureError> {
        ...
        if scheme != SignatureScheme::Secp256r1 {
            return Err(SignatureError::from_source(format!(
                "haneulpriv scheme flag is `{}`, expected `secp256r1`",
                scheme.name(),
            )));
        }
        let bytes: [u8; Self::LENGTH] = key.try_into().map_err(|_: Vec<u8>| {
            SignatureError::from_source("haneulpriv key has invalid length for secp256r1")
        })?;
        Ok(Self::new(bytes))
    }
    ...
    pub fn to_haneulprivkey(&self) -> Result<String, SignatureError> {
        ...
        crate::haneulpriv::encode(SignatureScheme::Secp256r1, &bytes)
    }
}
