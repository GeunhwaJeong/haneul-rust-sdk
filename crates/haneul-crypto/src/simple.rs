...
mod keypair {
    ...
    impl SimpleKeypair {
        ...
        /// accepted, matching the upstream `haneul-types` parser.
        ...
        /// the legacy keystore format used for entries of the Haneul CLI's
    ...
    impl SimpleVerifiyingKey {
        ...
        pub fn derive_address(&self) -> haneul_sdk_types::Address {
            match &self.inner {
                #[cfg(feature = "ed25519")]
                InnerVerifyingKey::Ed25519(verifying_key) => {
...
mod test {
    ...
    // `haneulprivkey` format, encoded as plain Base64 instead. This is the
    // encoding used for entries of the Haneul CLI's `haneul.keystore` file.
    mod base64 {
        use super::*;
        use haneul_sdk_types::SignatureScheme;
        ...
        #[cfg(feature = "bech32")]
        #[test]
        fn agrees_with_haneulprivkey_encoding() {
            // Both encodings carry the same payload, so converting between
            // them must be lossless.
            let keypair = SimpleKeypair::from_base64(UPSTREAM_ED25519_BASE64).unwrap();
            let haneulprivkey = keypair.to_haneulprivkey().unwrap();
            let round_tripped = SimpleKeypair::from_haneulprivkey(&haneulprivkey).unwrap();
