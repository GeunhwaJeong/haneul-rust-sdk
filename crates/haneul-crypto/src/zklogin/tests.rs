...
use haneul_sdk_types::ZkLoginAuthenticator;
...
fn pinned_circuit_proof_material() -> (ZkloginVerifier, ZkLoginInputs, ZkLoginInputs) {
    ...
    verifier.jwks_mut().insert(
        JwkId {
            iss: "https://oauth.haneul.io".to_string(),
            kid: "haneul-key-id".to_string(),
        },
        v1_jwk,
    );
    verifier.jwks_mut().insert(
        JwkId {
            iss: "https://jwt-tester.haneul-labs.com".to_string(),
            kid: "haneul-key-id-8192".to_string(),
        },
        v2_jwk,
    );
...
fn zklogin_verify_v1_v2_scenarios() {
    ...
    let key = test_eph_key();
    let signature: haneul_sdk_types::SimpleSignature = key.sign(&message.signing_digest());
...
fn zklogin_all_inputs_hash_matches_fastcrypto() {
    ...
    for (inputs, (iss, kid), version, expected) in [
        (
            &v1_inputs,
            ("https://oauth.haneul.io", "haneul-key-id"),
            CircuitVersion::V1,
            "10694871766605632598609552840913903724536924605230723954224229765332817156326",
        ),
        (
            &v2_inputs,
            ("https://jwt-tester.haneul-labs.com", "haneul-key-id-8192"),
            CircuitVersion::V2,
