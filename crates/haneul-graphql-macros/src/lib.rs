//! Compile-time validated macros for the Haneul GraphQL API.
//!
//! Two macros, both validated against the embedded Haneul GraphQL schema:
//!
...
//! [`haneul-graphql`](https://docs.rs/haneul-graphql).
//!
//! # Quick Start
//!
//! ```no_run
...
/// Validate a GraphQL query or mutation against the embedded Haneul schema at
...
fn derive_query_response_impl(input: DeriveInput) -> Result<TokenStream2, syn::Error> {
    ...
    // If a custom schema path is provided, load it; otherwise use the embedded Haneul schema.
    let loaded_schema = if let Some(path) = &parsed.schema {
        // Resolve path relative to the crate's directory.
        // HANEUL_GRAPHQL_SCHEMA_DIR is used by trybuild tests (which run from a temp directory).
        let base_dir = std::env::var("HANEUL_GRAPHQL_SCHEMA_DIR")
