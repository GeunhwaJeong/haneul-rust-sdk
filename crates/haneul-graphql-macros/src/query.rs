...
//! `haneul-graphql` crate ships a `macro_rules!` wrapper that nests this macro
...
/// into its error messages (e.g. `error at <haneul schema>:42:5`); it does not
/// open any file. The actual SDL bytes come from [`crate::schema::SCHEMA_SDL`].
const SCHEMA_DIAGNOSTIC_LABEL: &str = "<haneul schema>";
...
static VALIDATED_SCHEMA: LazyLock<Result<Valid<Schema>, String>> = LazyLock::new(|| {
    Schema::parse_and_validate(crate::schema::SCHEMA_SDL, SCHEMA_DIAGNOSTIC_LABEL)
        .map_err(|e| format!("Failed to parse Haneul GraphQL schema: {e}"))
});
