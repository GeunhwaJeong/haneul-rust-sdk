...
#[derive(Debug, Clone)]
pub struct Balance {
    /// The coin type (e.g., `0x2::haneul::HANEUL`).
...
impl Client {
    ...
    ///
    ///     // Get HANEUL balance using the helper
    ///     let haneul_balance = client.get_balance(owner, &StructTag::haneul()).await?;
    ///     if let Some(bal) = haneul_balance {
    ///         println!("HANEUL balance: {}", bal.total_balance);
    ///     }
    ///
    ...
    pub async fn get_balance(
        ...
        const QUERY: &str = graphql_query!(
            "query($owner: HaneulAddress!, $coinType: String!) {
    ...
    async fn fetch_balances_page(
        ...
        const QUERY: &str = graphql_query!(
            "query($owner: HaneulAddress!, $after: String) {
                address(address: $owner) {
...
mod tests {
    ...
    async fn test_get_balance_found() {
        ...
        let result = client.get_balance(owner, &StructTag::haneul()).await;
        assert!(result.is_ok());
        ...
        assert_eq!(balance.coin_type, StructTag::haneul());
    ...
    async fn test_list_balances_multiple() {
                ...
                "data": {
                    "address": {
                        "balances": {
                            ...
                            "nodes": [
                                {
                                    "coinType": { "repr": "0x2::haneul::HANEUL" },
                                    "totalBalance": "1000000000"
                                },
                                {
        ...
        assert_eq!(bal1.coin_type, StructTag::haneul());
