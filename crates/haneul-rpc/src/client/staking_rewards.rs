...
pub struct DelegatedStake {
    /// ObjectId of this StakedHaneul delegation.
...
impl Client {
    ...
    pub async fn list_delegated_stake(&mut self, address: &Address) -> Result<Vec<DelegatedStake>> {
        const STAKED_HANEUL_TYPE: &str = "0x3::staking_pool::StakedHaneul";
        ...
        let mut list_request = ListOwnedObjectsRequest::default()
            ...
            .with_read_mask(FieldMask::from_str("contents"))
            .with_object_type(STAKED_HANEUL_TYPE);
        ...
        loop {
            ...
            // with the fetched StakedHaneul objects, attempt to calculate the rewards and create a
    ...
    async fn try_create_delegated_stake_info(
        ...
        let staked_haneuls = maybe_staked_haneul
            ...
            .map(|o| {
                o.contents()
                    .deserialize::<StakedHaneul>()
                    .map_err(Into::into)
                    .map_err(tonic::Status::from_error)
            })
            .collect::<Result<Vec<StakedHaneul>>>()?;
