...
fn test_transaction() -> (proto::Transaction, String) {
    let transaction = haneul_sdk_types::Transaction {
        ...
        sender: haneul_sdk_types::Address::ZERO,
        gas_payment: haneul_sdk_types::GasPayment {
            objects: Vec::new(),
            owner: haneul_sdk_types::Address::ZERO,
            price: 1,
            budget: 1,
        },
        expiration: haneul_sdk_types::TransactionExpiration::None,
    };
...
async fn execution_error_without_commit_surfaces() {
    ...
    match err {
        haneul_rpc::client::ExecuteAndWaitError::RpcError(status) => {
...
async fn missing_transaction_fails_without_rpc() {
    ...
    assert!(matches!(
        err,
        haneul_rpc::client::ExecuteAndWaitError::MissingTransaction
    ));
}
