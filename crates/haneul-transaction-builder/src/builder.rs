...
///
/// [ptb]: https://docs.haneul.io/concepts/transactions/prog-txn-blocks
///
/// # Example
///
/// ```
...
pub(crate) enum ResolvedArgument {
    ...
    Resolved(haneul_sdk_types::Argument),
}
...
pub(crate) enum InputArg {
    ...
    Pure(Vec<u8>),
    Object(ObjectInput),
    FundsWithdrawal(haneul_sdk_types::FundsWithdrawal),
}
...
impl TransactionBuilder {
    ...
    pub fn funds_withdrawal(&mut self, coin_type: TypeTag, amount: u64) -> Argument {
        let funds_withdrawal = haneul_sdk_types::FundsWithdrawal::new(
            amount,
            coin_type,
    ...
    ///
    /// This adds a [`FundsWithdrawal`](haneul_sdk_types::FundsWithdrawal) input and calls
    ...
    ///     )
    ///     .with_type_args(vec!["0x2::haneul::HANEUL".parse().unwrap()]),
    ///     vec![],
    /// );
    /// ```
    ...
    ///
    /// let mut tx = TransactionBuilder::new();
    /// let coin = tx.intent(Coin::haneul(1_000_000_000));
    ...
    ///
    /// let mut tx = TransactionBuilder::new();
    /// let coin = tx.coin(StructTag::haneul(), 1_000_000_000);
    /// ```
    #[cfg(feature = "intents")]
    #[cfg_attr(doc_cfg, doc(cfg(feature = "intents")))]
    pub fn coin(&mut self, coin_type: haneul_sdk_types::StructTag, amount: u64) -> Argument {
    ...
    ///
    /// let mut tx = TransactionBuilder::new();
    /// let bal = tx.balance(StructTag::haneul(), 1_000_000_000);
    /// ```
    #[cfg(feature = "intents")]
    #[cfg_attr(doc_cfg, doc(cfg(feature = "intents")))]
    pub fn balance(&mut self, coin_type: haneul_sdk_types::StructTag, amount: u64) -> Argument {
    ...
    pub fn try_build(mut self) -> Result<Transaction, Error> {
        ...
        // Gas payment
        let gas_payment = haneul_sdk_types::GasPayment {
            objects: self
                .gas
                .iter()
        ...
        for (id, input) in unresolved_inputs {
            let arg = match input {
                InputArg::Gas => haneul_sdk_types::Argument::Gas,
                InputArg::Pure(value) => {
                    resolved_inputs.push(haneul_sdk_types::Input::Pure(value));
                    haneul_sdk_types::Argument::Input(resolved_inputs.len() as u16 - 1)
                }
                ...
                InputArg::FundsWithdrawal(funds_withdrawal) => {
                    resolved_inputs
                        .push(haneul_sdk_types::Input::FundsWithdrawal(funds_withdrawal));
                    haneul_sdk_types::Argument::Input(resolved_inputs.len() as u16 - 1)
                }
            };
        ...
        for (id, command) in self.commands {
            ...
            let arg = haneul_sdk_types::Argument::Result(resolved_commands.len() as u16 - 1);

            *self.arguments.get_mut(&id).unwrap() = ResolvedArgument::Resolved(arg);
        }

        Ok(Transaction {
            kind: haneul_sdk_types::TransactionKind::ProgrammableTransaction(
                haneul_sdk_types::ProgrammableTransaction {
    ...
    #[cfg(feature = "intents")]
    #[cfg_attr(doc_cfg, doc(cfg(feature = "intents")))]
    pub async fn build(mut self, client: &mut haneul_rpc::Client) -> Result<Transaction, Error> {
        ...
        for (id, input) in unresolved_inputs {
            let arg = match input {
                ...
                InputArg::FundsWithdrawal(funds_withdrawal) => {
                    resolved_inputs
                        .push(haneul_sdk_types::Input::FundsWithdrawal(funds_withdrawal).into());
                    haneul_sdk_types::Argument::Input(resolved_inputs.len() as u16 - 1)
                }
            };
...
impl Argument {
    ...
    ) -> Result<haneul_sdk_types::Argument, Result<usize, Error>> {
    ...
    fn try_resolve_many(
        arguments: &[Self],
        resolved_arguments: &BTreeMap<usize, ResolvedArgument>,
    ) -> Result<Vec<haneul_sdk_types::Argument>, Result<usize, Error>> {
        arguments
            .iter()
...
impl Command {
    fn try_resolve(
        &self,
        resolved_arguments: &BTreeMap<usize, ResolvedArgument>,
    ) -> Result<haneul_sdk_types::Command, Result<usize, Error>> {
        use haneul_sdk_types::Command as C;
        ...
        let cmd = match &self.kind {
            CommandKind::MoveCall(MoveCall {
                ...
                type_arguments,
                arguments,
            }) => C::MoveCall(haneul_sdk_types::MoveCall {
                package: *package,
            ...
            CommandKind::TransferObjects(TransferObjects { objects, address }) => {
                C::TransferObjects(haneul_sdk_types::TransferObjects {
            ...
            CommandKind::SplitCoins(SplitCoins { coin, amounts }) => {
                C::SplitCoins(haneul_sdk_types::SplitCoins {
            ...
            CommandKind::MergeCoins(MergeCoins {
                coin,
                coins_to_merge,
            }) => C::MergeCoins(haneul_sdk_types::MergeCoins {
                coin: coin.try_resolve(resolved_arguments)?,
            ...
            CommandKind::Upgrade(Upgrade {
                ...
                package,
                ticket,
            }) => C::Upgrade(haneul_sdk_types::Upgrade {
                modules: modules.to_owned(),
                dependencies: dependencies.to_owned(),
                package: *package,
...
impl From<&haneul_sdk_types::Object> for ObjectInput {
    fn from(object: &haneul_sdk_types::Object) -> Self {
        let input = Self::new(object.object_id())
            .with_version(object.version())
            .with_digest(object.digest());

        match object.owner() {
            haneul_sdk_types::Owner::Address(_) => input.as_owned(),
            haneul_sdk_types::Owner::Object(_) => input,
            haneul_sdk_types::Owner::Shared(version) => input.with_version(*version).as_shared(),
            haneul_sdk_types::Owner::Immutable => input.as_immutable(),
            haneul_sdk_types::Owner::ConsensusAddress { start_version, .. } => {
...
// private conversions
impl ObjectInput {
    fn try_into_object_reference(&self) -> Result<haneul_sdk_types::ObjectReference, Error> {
        if matches!(self.kind, Some(ObjectKind::ImmutableOrOwned) | None)
            && let Some(version) = self.version
            && let Some(digest) = self.digest
        {
            Ok(haneul_sdk_types::ObjectReference::new(
                self.object_id,
                version,
                digest,
            ))
    ...
    fn try_into_input(&self) -> Result<haneul_sdk_types::Input, Error> {
        let input = match self {
            ...
            | Self {
                ...
                digest: Some(digest),
                mutable: None,
            } => haneul_sdk_types::Input::ImmutableOrOwned(haneul_sdk_types::ObjectReference::new(
            ...
            Self {
                ...
                digest: Some(digest),
                ..
            } => haneul_sdk_types::Input::Receiving(haneul_sdk_types::ObjectReference::new(
                *object_id, *version, *digest,
            )),
            ...
            | Self {
                ...
                digest: None,
                mutable: Some(mutable),
            } => haneul_sdk_types::Input::Shared(haneul_sdk_types::SharedInput::new(
    ...
    #[cfg(feature = "intents")]
    fn to_input_proto(&self) -> haneul_rpc::proto::haneul::rpc::v2::Input {
        use haneul_rpc::proto::haneul::rpc::v2::input::InputKind;

        let mut input =
            haneul_rpc::proto::haneul::rpc::v2::Input::default().with_object_id(self.object_id);
        match &self.kind {
    ...
    fn try_into_object_reference_proto(
        &self,
    ) -> Result<haneul_rpc::proto::haneul::rpc::v2::ObjectReference, Error> {
        if !matches!(self.kind, Some(ObjectKind::ImmutableOrOwned) | None) {
            return Err(Error::WrongGasObject);
        }

        let mut input = haneul_rpc::proto::haneul::rpc::v2::ObjectReference::default()
    ...
    pub(crate) fn try_from_object_proto(
        object: &haneul_rpc::proto::haneul::rpc::v2::Object,
    ) -> Result<Self, Error> {
