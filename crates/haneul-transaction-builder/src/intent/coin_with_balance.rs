...
//!
//! Haneul accounts hold funds in two forms: **coin objects** (`Coin<T>`)
...
//!
//! if HANEUL and gas coin enabled:
...
///
/// // Request 1 HANEUL (uses gas coin by default)
/// let coin = Coin::haneul(1_000_000_000);
///
...
impl Coin {
    ...
    /// use haneul_transaction_builder::intent::Coin;
    ///
    /// let coin = Coin::new(StructTag::haneul(), 1_000_000_000);
    /// ```
    ...
    /// Shorthand for requesting a HANEUL coin with the given balance.
    ///
    ...
    /// use haneul_transaction_builder::intent::Coin;
    ///
    /// let one_haneul = Coin::haneul(1_000_000_000);
    /// ```
    pub fn haneul(balance: u64) -> Self {
        Self {
            coin_type: StructTag::haneul(),
            balance,
            use_gas_coin: true,
        }
    }
    ...
    ///
    /// This is only meaningful when the coin type is HANEUL. Pass `false` to force the resolver
    /// to select non-gas HANEUL coins instead.
    ///
    /// ```
    /// use haneul_transaction_builder::intent::Coin;
    ///
    /// // Don't touch the gas coin -- select other HANEUL coins instead
    /// let coin = Coin::haneul(1_000_000_000).with_use_gas_coin(false);
    /// ```
...
///
/// // Request 1 HANEUL as a Balance<HANEUL>
/// let bal = Balance::haneul(1_000_000_000);
///
...
impl Balance {
    ...
    /// use haneul_transaction_builder::intent::Balance;
    ///
    /// let bal = Balance::new(StructTag::haneul(), 1_000_000_000);
    /// ```
    pub fn new(coin_type: StructTag, balance: u64) -> Self {
        Self {
            coin_type,
    ...
    /// Shorthand for requesting a HANEUL balance with the given amount.
    ///
    ...
    /// use haneul_transaction_builder::intent::Balance;
    ///
    /// let one_haneul = Balance::haneul(1_000_000_000);
    /// ```
    pub fn haneul(balance: u64) -> Self {
        Self {
            coin_type: StructTag::haneul(),
    ...
    /// use haneul_transaction_builder::intent::Balance;
    ///
    /// let bal = Balance::haneul(1_000_000_000).with_use_gas_coin(false);
    /// ```
    pub fn with_use_gas_coin(self, use_gas_coin: bool) -> Self {
        Self {
            use_gas_coin,
        ...
        for (id, intent) in builder.intents.extract_if(.., |_id, intent| {
            ...
            if let Some(coin) = intent.downcast_ref::<Coin>() {
                if coin.balance == 0 {
                    ...
                    let coin_type = if coin.coin_type == StructTag::haneul() && coin.use_gas_coin {
                        CoinType::Gas
                    } else {
                ...
                if bal.balance == 0 {
                    ...
                    let coin_type = if bal.coin_type == StructTag::haneul() && bal.use_gas_coin {
                        CoinType::Gas
                    } else {
...
impl Resolver {
    ...
    ///   HANEUL buffer for gas fees. No remainder handling.
    ...
    async fn resolve_merge_and_split(
        ...
        let (coin_type, use_gas_coin) = match coin_type {
            CoinType::Gas => (StructTag::haneul(), true),
    ...
    fn prepare_gas_coin_sources(
        ...
        // sender: Address,
        address_balance: u64,
        // chain_info: (Option<haneul_sdk_types::Digest>, Option<u64>),
    ) -> (Argument, Vec<Argument>) {
        let gas = builder.gas();
        let mut deps = Vec::new();
        ...
        // {
        //     let reservation = haneul_sdk_types::ObjectReference::coin_reservation(
        //         coin_type,
        //         reservation_amount,
        //         epoch,
        //         chain_id,
